$(function(event) {
  //  $(document).bind("contextmenu", function(e) {
   ...
   // });
    ...
    $.ajax({ url: "/runtimeWeb.ashx", dataType: "json", type: "POST", data: { operatetype: "getOnlineRoleId" },
        success: function(data) {
            if (data.IsSuccess) {
                var roleId = data.Data;
                $.ajax({ url: "runtimeWeb.ashx", async: false, dataType: "json", type: "POST", data: { operatetype: "getPowerHtmlForRole", roleId: escape(roleId) },
                    success: function(data) {
                        if (data.IsSuccess) {
                            ...
                            $("#treeDiv").find("a").bind("click", function(event) {
                                ...
                                $.ajax({ url: "/runtimeWeb.ashx", dataType: "json", type: "POST", data: { operatetype: "getPageColumns", modelId: escape(modelId) },
                                    success: function(data) {
                                        if (data.IsSuccess) {
    ...
    function createDataview(dataviewId) {
        ...
        $(".lineClass").remove();
        $.ajax({ url: "/runtimeWeb.ashx", dataType: "json", type: "POST", data: { operatetype: "getDataviewById", dvId: escape(dataviewId) },
            success: function(data) {
                if (data.IsSuccess) {
                    dv = data.Data;
    ...
    $("#saveDvBt").bind("click", function() {
        ...
        $.ajax({ url: "/runtimeWeb.ashx", type: "POST", dataType: "json", data: { operatetype: "saveDataview", dvStr: escape(dvStr) },
            success: function(data) {
                if (data.IsSuccess) {
                    alert("已经成功保存视图！！");
    ...
    $("#importDvBt").bind("click", function() {
        $.ajax({ url: "/runtimeWeb.ashx", type: "POST", dataType: "json", data: { operatetype: "getAllDataview" },
            success: function(data) {
                if (data.IsSuccess) {
                    ...
                    for (var i = 0; i < data.Data.length; i++) {
                        (function() {
                            ...
                            $(delTd).css("border", "1px solid black");
                            $(delTd.childNodes[0]).bind("click", function() {
                                $.ajax({ url: "/runtimeWeb.ashx", type: "POST", dataType: "json", data: { operateType: "delDataview", id: dataview.id }
                                   , success: function(data) {
                                       if (data.Success) {
                                           $(row).remove();
                                       }
