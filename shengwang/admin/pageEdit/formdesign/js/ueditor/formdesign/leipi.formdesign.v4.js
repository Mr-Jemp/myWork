 ...
 * @example
 * ```javascript
 * editor.execCommand( 'dynamictree');
 * ```
 */
UE.plugins['dynamictree'] = function () {
	var me = this,thePlugins = 'dynamictree';
	me.commands[thePlugins] = {
		execCommand:function () {
			var dialog = new UE.ui.Dialog({
				iframeUrl:this.options.UEDITOR_HOME_URL + UE.leipiFormDesignUrl+'/dynamictree.html',
				name:thePlugins,
				editor:this,
				title: '页面树',
	...
	me.addListener( 'mouseover', function( t, evt ) {
		...
		if ( /input/ig.test( el.tagName ) && leipiPlugins==thePlugins) {
			var html = popup.formatHtml(
				'<nobr>页面树: <span onclick=$$._edittext() class="edui-clickable">编辑</span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>' );
			if ( html ) {
...
UE.plugins['expression'] = function () {
    ...
    me.commands[thePlugins] = {
        execCommand:function () {
            var dialog = new UE.ui.Dialog({
                iframeUrl:this.options.UEDITOR_HOME_URL + UE.leipiFormDesignUrl+'/expression.html',
                name:thePlugins,
                editor:this,
                title: '表达式',
    ...
    me.addListener( 'mouseover', function( t, evt ) {
        ...
        if ( /input/ig.test( el.tagName ) && leipiPlugins==thePlugins) {
            var html = popup.formatHtml(
                '<nobr>表达式: <span onclick=$$._edittext() class="edui-clickable">编辑</span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>' );
            if ( html ) {
                popup.getDom( 'content' ).innerHTML = html;
                popup.anchorEl = el;
                popup.showAnchor( popup.anchorEl );
 ...
 * @example
 * ```javascript
 * editor.execCommand( 'sublist');
 * ```
 */
UE.plugins['sublist'] = function () {
    var me = this,thePlugins = 'sublist';
    me.commands[thePlugins] = {
        execCommand:function () {
            var dialog = new UE.ui.Dialog({
                iframeUrl:this.options.UEDITOR_HOME_URL + UE.leipiFormDesignUrl+'/sublist.html',
                name:thePlugins,
                editor:this,
                title: '子列表',
                cssRules:"width:600px;height:270px;",
    ...
    me.addListener( 'mouseover', function( t, evt ) {
        ...
        if ( /input/ig.test( el.tagName ) && leipiPlugins==thePlugins) {
            var html = popup.formatHtml(
                '<nobr>子列表: <span onclick=$$._edittext() class="edui-clickable">编辑</span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>' );
            if ( html ) {
                popup.getDom( 'content' ).innerHTML = html;
                popup.anchorEl = el;
                popup.showAnchor( popup.anchorEl );
 ...
 * @example
 * ```javascript
 * editor.execCommand( 'free');
 * ```
 */
UE.plugins['free'] = function () {
	var me = this,thePlugins = 'free';
	me.commands[thePlugins] = {
		execCommand:function () {
			var dialog = new UE.ui.Dialog({
				iframeUrl:this.options.UEDITOR_HOME_URL + UE.leipiFormDesignUrl+'/free.html',
				name:thePlugins,
				editor:this,
				title: '树控件',
				cssRules:"width:600px;height:310px;",
 ...
 * @example
 * ```javascript
 * editor.execCommand( 'imagelist');
 * ```
 */
UE.plugins['imagelist'] = function () {
	var me = this,thePlugins = 'imagelist';
	me.commands[thePlugins] = {
		execCommand:function () {
			var dialog = new UE.ui.Dialog({
				iframeUrl:this.options.UEDITOR_HOME_URL + UE.leipiFormDesignUrl+'/imagelist.html',
				name:thePlugins,
				editor:this,
				title: '图片列表',
				cssRules:"width:600px;height:310px;",
	...
	me.addListener( 'mouseover', function( t, evt ) {
		...
		var el = evt.target || evt.srcElement;
        var leipiPlugins = el.getAttribute('leipiplugins');
		if ( /td/ig.test( el.tagName ) && leipiPlugins=="td_"+thePlugins) {
			
			var evt = document.createEvent('Event'); 
			evt.initEvent('click',true,true); 
			var par=el.parentNode.parentNode.parentNode;
	...
	me.addListener("click",function(t,evt)
	{
		...
		var leipiPlugins = el.getAttribute('leipiplugins');
		if(leipiPlugins!=thePlugins)return;
		var html = popup.formatHtml(
			'<nobr>表单跳转: <span onclick=$$._edittext() class="edui-clickable">编辑</span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>' );
		if ( html ) {
			popup.getDom( 'content' ).innerHTML = html;
			popup.anchorEl = el;
			popup.showAnchor( popup.anchorEl );
 ...
 * @example
 * ```javascript
 * editor.execCommand( 'uploadfile');
 * ```
 */
UE.plugins['uploadfile'] = function () {
	var me = this,thePlugins = 'uploadfile';
	me.commands[thePlugins] = {
		execCommand:function () {
			var dialog = new UE.ui.Dialog({
				iframeUrl:this.options.UEDITOR_HOME_URL + UE.leipiFormDesignUrl+'/uploadfile.html',
				name:thePlugins,
				editor:this,
				title: '文件上传',
				cssRules:"width:600px;height:310px;",
 ...
 * ```
 */
UE.plugins['formto'] = function () {
	var me = this,thePlugins = 'formto';
	me.commands[thePlugins] = {
		execCommand:function () {
			var dialog = new UE.ui.Dialog({
				iframeUrl:this.options.UEDITOR_HOME_URL + UE.leipiFormDesignUrl+'/formto.html',
				name:thePlugins,
				editor:this,
				title: '表单跳转',
				cssRules:"width:600px;height:310px;",
...
UE.registerUI('button_mobile_preview',function(editor,uiName){
    if(!this.options.toolleipi)
    {
        return false;
    }
    ...
    editor.registerCommand(uiName,{
        execCommand:function(){
            try {
                leipiFormDesign.fnReview('mobile');
            } catch ( e ) {
                alert('leipiFormDesign.fnReview() 手机预览异常');
            }
