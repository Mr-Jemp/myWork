...
class FlowdesignController extends HomeController {
    ...
    public function add_process()
    {
        ...
        $data = array(
           ...
           'settop'=>$top,
           'process_to'=>'',
		   'auto_sponsor_ids'=>$_SESSION['uid'],
		   'auto_sponsor_text'=>$_SESSION['username'],
    ...
    public function getformcot(){
        $form_id = intval(I('get.id'));
       $form= M('form')->field('id,form_name,content_data,fields')->where('id='.$form_id)->find();
        $form['content_data'] = $form['content_data']=='' ? array() : unserialize($form['content_data']);
        $html='';
        $form_plugins=C('FORM_PLUGINS');
        $i=0;
        foreach($form['content_data'] as $vo){
            if($vo['leipiplugins']=='checkboxs'){
                $html.='<tr>
              <th>'.$vo['title'].'</th>
              <td>'.$form_plugins[$vo['leipiplugins']].'</td>
              <td><label class="checkbox"><input type="checkbox" name="write_fields[]"  value="'.$vo['parse_name'].'" key="'.$i.'" id="write_'.$i.'"></label></td>
              <td><label class="checkbox"><input type="checkbox" name="secret_fields[]" value="'.$vo['parse_name'].'" key="'.$i.'" id="secret_'.$i.'"></label></td>
            </tr>';
            }else{
                $html.='<tr>
              <th>'.$vo['title'].'</th>
              <td>'.$form_plugins[$vo['leipiplugins']].'</td>
              <td><label class="checkbox"><input type="checkbox" name="write_fields[]"  value="'.$vo['name'].'" key="'.$i.'" id="write_'.$i.'"></label></td>
              <td><label class="checkbox"><input type="checkbox" name="secret_fields[]"  value="'.$vo['name'].'" key="'.$i.'" id="secret_'.$i.'"></label></td>
            </tr>';
            }
            $i++;
        }
    ...
    public function attribute()
    {
        ...
        // 流程使用的表单列表
       $cform_ids=$form_model->field('id,form_name')->select(trim($flow_one['form_id'],','));
        ...
        $this->assign('cform_ids',$cform_ids);
    ...
    public function save_attribute()
    {
        //print_R($_POST);exit;
        //start 避免出错，都列出来先
        $form_id=intval(I('post.cform_id'));//表单ID
        $powers=I('post.powers');
