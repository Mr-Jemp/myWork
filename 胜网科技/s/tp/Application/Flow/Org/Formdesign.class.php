...
class Formdesign
{
    ...
    public function parse_table($formid,$add_fields)
    {
        ...
        if($exist ===false)
        {
            ...
            $sql = "CREATE TABLE `".$tname."` (
              ...
              `foreign_id` int(10) unsigned NULL DEFAULT '0',
              ".$fields."
    ...
    * );
    */
    public function unparse_form($form,$form_data=array(),$controller=array(),$isdis_arr=array())
    {
        ...
        foreach($tpl_data as $key=>$value)
        {
			...
			$dis='';		
				//if(empty($isdis_arr) or !in_array($value["name"],$isdis_arr))$dis=' disabled="disabled"';	
				$value['value'] = isset($form_data[$value['name']]) ? $form_data[$value['name']] : $value['value'];
			/*}//通用*/
            
            switch($value['leipiplugins'])
            {
                case 'text':
                    $temp_html = '<input type="text" value="'.$value['value'].'"  name="'.$value['name'].'"  style="'.$value['style'].'" '.$dis.'/>';
                    break;
                case 'textarea':
                        $temp_html = '<textarea  name="'.$value['name'].'" id="'.$value['name'].'" value="'.$value['value'].'" style="'.$value['style'].'"'.$dis.'>';
                ...
                case 'radios':
                    ...
                    foreach($value['options'] as $item)
                    {
                        ...
                        $temp_html .='<input type="radio" name="'.$value['name'].'" value="'.$item['value'].'" '.$checked.' '.$dis.'>'.$item['value'].'&nbsp;';
                    }
                    break;
                case 'checkboxs':
                    $temp_html = '';
                    ...
                    foreach($value['options'] as $item)
                    {
                        ...
                        $temp_html .='<input type="checkbox" name="'.$item['name'].'" value="'.$item['value'].'" '.$checked.' '.$dis.'>'.$item['value'].'&nbsp;';
