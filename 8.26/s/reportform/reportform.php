	...
	$(function(){
		...
		if(formeditor==null){
			...
			for(var i=0;i<out.length;i++){
				...
				formeditor=formeditor.replace(out[i],value);
			}
...
<div class="container">
	<div id="design_content">
		<p class="title">【<?=$row['form_name']?><?=strlen($name)>0?'->'.$name:''?>】 <a href="javascript:;" class="back">数据设置</a> | 查看数据</p>
		<p id="content"><?php echo str_replace('<table','<table class="table table-bordered" ',$row['content']);?></p>
		<p class="title">&nbsp;<input id="return" type="button" value=" 返 回 "></p>
	</div>
</div>
</body>
<script>
	$(function()
	{
		...
		{
			var url='/s/reportform/submitreportform.php?formid=<?=$formid?>';
