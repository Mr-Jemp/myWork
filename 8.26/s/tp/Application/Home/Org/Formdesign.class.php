    ...
    */
    public function parse_table($formid,$add_fields)
    {
        ...
        if(!$exist)
        {
            ...
            {
                $fields .='`'.$value['name'].'` '.self::field_type_sql($value['leipiplugins']).' comment \''.$value['title'].'\',';
            }
        ...
        }else
        {
            ...
            foreach($add_fields as $value)
            {
               if(!in_array($value['name'],$fields))
                {
                    $sql.= $dot . "ADD COLUMN `".$value['name']."` ".self::field_type_sql($value['leipiplugins']).' comment \''.$value['title'].'\'';
                    $dot = ',';
                }else{
					$sql.= $dot . " CHANGE `".$value['name']."` `".$value['name']."` ".self::field_type_sql($value['leipiplugins'])." COMMENT '".$value['title']."'";
					$dot = ',';
				}
            }
            $sql .=';';
        ...
        foreach($tpl_data as $key=>$value)
        {
            ...
            switch($value['leipiplugins'])
            {
               ...
               case 'qrcode'://未处理生成二维码
                    ...
                    break;
				case 'uploadfile':
					include $_SERVER['DOCUMENT_ROOT']."/s/inc/control.uploadfile.php";
					break;
				case 'free':
					include $_SERVER['DOCUMENT_ROOT']."/s/inc/control.plaintree.php";
					break;
				case 'dynamictree':
					include $_SERVER['DOCUMENT_ROOT']."/s/inc/control.dynamictree.php";
					break;
				case 'sublist':
					include $_SERVER['DOCUMENT_ROOT']."/s/inc/control.sublist.php";
					break;
				case 'expression':
					include $_SERVER['DOCUMENT_ROOT']."/s/inc/control.expression.php";
					break;
                case 'listctrl':
    ...
    public function macros_parse($data,$def_value='',$controller=array())
    {
        ...
        switch($data['orgtype'])
        {
            ...
            case 'sys_realname':
                ...
                $tpl = str_replace('{macros}',$def_value,$tpl);
                break;
			case 'sys_date_select';
				include $_SERVER['DOCUMENT_ROOT']."/s/inc/control.macros.date_select.php";
				break;
			case 'sys_data_arithmetic':
				$result="";
				$tmp_fun_type["统计"]="count";
				$tmp_fun_type["合计"]="sum";
				$tmp_fun_type["平均值"]="avg";
				$tmp_fun_type["最大值"]="max";
				$tmp_fun_type["最小值"]="min";
				$tmp_formid=$data['orgformid'];
				$tmp_year=$data['orgyear'];
				$tmp_month=$data['orgmonth'];
				$tmp_field=$data['orgfield'];
				$tmp_fun=$tmp_fun_type[$data['orgfun']];
				if(strlen($data['orgfun'])<1)$tmp_fun='e235fbf4fc784ab4';
				
				if(!preg_match('/^\d+$/',$tmp_formid))$tmp_formid='(e235fbf4fc784ab4)';else $tmp_formid="ht_form_data_$tmp_formid";
				$sql="select IFNULL($tmp_fun($tmp_field),0) as result from $tmp_formid where id<>0";
				if(preg_match('/^[1-9]{1}\d{3}$/',$tmp_year))
				{
					$startm='01';$endm='12';
					if(preg_match('/^\d{2}$/',$tmp_month))$startm=$endm=$tmp_month;
					
					$endd=date('d',strtotime("$tmp_year-$endm-01 +1 month -1 day"));
					$sql.=" and dateline>=UNIX_TIMESTAMP('$tmp_year-$startm-01 00:00:00')"
					." and dateline<=UNIX_TIMESTAMP('$tmp_year-$endm-$endd 23:59:59')";
				...
				$tpl = str_replace('{macros}',$result,$tpl);
				break;
            default:
